...
export default function Home() {
  ...
  // Troca de vídeo: reseta estados e força autoplay
  useEffect(() => {
    setVideoTitle(videoList[currentVideoIndex].title);
    setCurrentTime(0);
    setDuration(0);
    setIsVideoReady(false);
    setError(null);
    setPlaying(true); // Sempre tenta tocar ao trocar
  ...
  // Sempre que playing for true e o vídeo estiver pronto, tenta dar play
  useEffect(() => {
    const video = videoRef.current;
    if (video && isVideoReady && playing) {
      video.play().catch(() => {
        setPlaying(false);
      });
    }
  }, [isVideoReady, playing]);
  ...
  // Atualiza currentTime com requestAnimationFrame e também com timeupdate
  useEffect(() => {
    const video = videoRef.current;
    if (!video) return;
    ...
    const onLoadedMetadata = () => {
      ...
      setCurrentTime(0); // Garante que o tempo zere ao carregar
    ...
    const onEnded = () => {
      ...
      setTimeout(() => {
        ...
        // setPlaying(true); // Não precisa, pois o useEffect de troca já seta
      }, 200);
    };
    ...
    // Se o vídeo já está tocando, inicia o loop
    if (!video.paused && rafIdRef.current === null) {
      rafIdRef.current = requestAnimationFrame(updateTime);
    }
  ...
  }, [currentVideoIndex, duration, playing]);
    ...
    <div className="w-screen h-screen bg-[#121212] flex justify-center items-center font-sans text-white">
      <div className="group bg-[#1a1a1a] rounded-l-[20px] h-[580px] w-24 hover:w-[300px] p-6 shadow-lg shadow-black/50 flex flex-col transition-all duration-300 ease-in-out overflow-hidden">
        <h2 className="text-xl font-bold mb-4 border-b border-gray-700 pb-2 flex-shrink-0 whitespace-nowrap">
          <FaMusic className="inline-block mr-0 group-hover:mr-3 transition-all duration-300" />
        ...
        <div className="flex items-center w-full mb-[20px]">
          ...
          >
            <div className="w-full h-[4px] bg-[#333] rounded-[2px] relative group-hover:h-[6px] transition-all duration-200">
              ...
              >
                <div className="w-3 h-3 bg-[#ff0050] rounded-full shadow-md border-2 border-white" />
              </div>
            </div>
          </div>
